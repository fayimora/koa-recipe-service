...
mongodb.connect(url, (err, db) => {
  ...
  const prefillIngredients = Promise.map(sampleData.ingredients, function(
    ingredient
  ) {
    return IngredientCollection.insert(ingredient);
  });

  return Promise.all([prefillUsers, prefillRecipes, prefillIngredients])
    .then(results => {
      let sum = 0;
    ...
    .catch(error => {
      if (error && error.code === 11000) {
        console.error(
          `Duplicate entries detected!!!, ${colors.red("Drop Database")}, do ${colors.yellow("npm run start-dev")} and give this script another go :)`
        );
        process.exit(0);
      }
      throw error;
